...
namespace BookStore_Data_Layer.Data
{
    ...
    "Data Source=.\\SQLEXPRESS;Integrated Security=True;TrustServerCertificate=True;Database=BookStoreDB;"
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Purchase>().ToTable("Orders").HasKey(p => p.Id);
            modelBuilder.Entity<Purchase>().Property(p => p.Id).HasColumnName("OrderID");
            modelBuilder.Entity<Purchase>().Property(p => p.CustomerId).HasColumnName("CustomerID");
            modelBuilder.Entity<Purchase>().Property(p => p.PurchaseDate).HasColumnName("OrderDate");
            modelBuilder.Entity<Purchase>().HasMany(p => p.ORDERDETAIL).WithOne(od => od.Order).HasForeignKey(o => o.OrderID);
